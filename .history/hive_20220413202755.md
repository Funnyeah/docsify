...
```sql
select ~city_id from ai.dws_ai_jw_block_flag_da        运算符+、-、～、*、/、%、DIV 字段    (/为浮点数除法，eg.select 10 / 3  返回3.3333；div为整除，eg.select 10 div 3  返回3)
Select 5 | 7  --  输出13  二进制按位或运算        &  为二进制按位与运算
select * from ai.dws_ai_jw_block_flag_da where city_id is null .  字段 is null、字段 is not null（为空、不为空）
