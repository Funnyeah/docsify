...
Select ltrim(string A)  ,rtrim(string A),   删除字符串中左右空格 
select trim(' 13 df  ‘)     删除前后所有空格，不删除中间
select LPAD('lxh',4,'0’), lpad(‘lxh’,2,’0’), RPAD('df',5,0)     返回 ‘0lxh’ ,’lx’ ,’df000’  填充，若返回的长度小于原先字符串的长度，则不填充，截取所返回长度的原字符串
