...
### 比较运算符
```sql
Select  1= 2  和  Select  1== 2     均返回False，同义运算符     
Select  1<=> 2    非NULL值数据时等价于 = 和 ==   ；若有均为NULL 返回 True ，一方为NULL返回False
Select 1<> 2    这个是hive sql里的不等于号   等价于 != ，符号间不能有空格    若有一方为NULL、或者均为NULL, 则返回NULL

Select  1 < 、<= 、> 、>= 2.    大于、小于、大小等于符号， 符号间不能有空格,  若有一方为NULL、或者均为NULL, 则返回NULL.  
```
