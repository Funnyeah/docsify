...
select * from ai.dws_ai_jw_block_flag_da m where event_day = '20210607’  order by  rand(12)   全局随机 rand(可以指定随机种子)
select exp(flag) from ai.dws_ai_jw_block_flag_da    对某列数据取e^col
select ln(flag) from ai.dws_ai_jw_block_flag_da    对某列数据取ln(col)
log10( )     log2( )    log(3,5)   同理
pow(2,3)  返回2^3        sqrt(4)  返回2 开根号
bin( 5)  返回101  返回二进制       hex(16)  返回10    十六进制
factorial(2)  返回2的阶乘
